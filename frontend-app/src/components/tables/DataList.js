...
function DataList({ 
  ...
  const renderErrorMessages = () => {
    ...
    if (postError) errors.push({ type: 'post', message: `Add error: ${postError}` });
    if (updateError) errors.push({ type: 'update', message: `Update error: ${updateError}` });
  ...
  return (
    <div className="data-table-container">
            ...
            return (
              <tr key={key} {...headerGroupProps}>
                {headerGroup.headers.map(column => {
                  ...
                  return (
                    <th key={key} {...columnProps} style={{ width: column.width }}>
      ...
      {rows.length === 0 && (
        <div className="text-center p-3">No data available. Click the "+" button to add a new record.</div>
      )}
    </div>
  );
}
